...
class noteRoutes {
  ...
  private routes = () => {
    ...
    this.router.post(
      '',
      Auth,
      this.NoteValidator.validateNote,
      this.NoteController.createNote
    );

    // Route to get all Notes of a user
    this.router.get('/', Auth, this.NoteController.getAllNotes);

    // Route to get a note by its ID
    this.router.get('/:id', Auth, this.NoteController.getNoteById);
    ...
    this.router.put(
      ...
      Auth,
      this.NoteValidator.validateNote,
      this.NoteController.ArchiveNote
    );
    ...
    this.router.delete(
      ...
      Auth,
      this.NoteValidator.validateNote,
      this.NoteController.deleteNoteForever
    );
  };
